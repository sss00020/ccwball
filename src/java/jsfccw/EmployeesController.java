...
import sessionccw.EmployeesFacade;
...
public class EmployeesController implements Serializable {
    ...
    @EJB
    private sessionccw.EmployeesFacade ejbFacade;
    ...
    public Employees getSelected() {
        if (current == null) {
            ...
            current.setEmployeesPK(new ccw.EmployeesPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bunccw").getString("EmployeesCreated"));
    ...
    public String update() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bunccw").getString("EmployeesUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bunccw").getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bunccw").getString("EmployeesDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bunccw").getString("PersistenceErrorOccured"));
        }
    }
    ...
    public Employees getEmployees(ccw.EmployeesPK id) {
    ...
    public static class EmployeesControllerConverter implements Converter {
        ...
        ccw.EmployeesPK getKey(String value) {
            ccw.EmployeesPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new ccw.EmployeesPK();
            key.setEmpId(values[0]);
            key.setMsgId(values[1]);
            key.setMemId(values[2]);
            key.setProNo(values[3]);
            return key;
        }

        String getStringKey(ccw.EmployeesPK value) {
